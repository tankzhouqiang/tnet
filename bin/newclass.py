...
def createClassH(path, classname):
    ...
    if not ns:
        return False
    context = ""
    context = context + "#ifndef TNET_" + classname.upper() + "_H" + "\n"
    context = context + "#define TNET_" + classname.upper() + "_H" + "\n"
    context = context + "" + "\n"
    context = context + "#include <tnet/common/Common.h>" + "\n"
    context = context + "" + "\n"
    ...
    context = context + "};" + "\n"
    context = context + "" + "\n"
    context = context + "TNET_TYPEDEF_PTR(" + classname +");" + "\n\n"
    context = context + "TNET_END_NAMESPACE(" + ns + ");" + "\n"
    context = context + "" + "\n"
    context = context + "#endif //TNET_" + classname.upper() + "_H" + "\n"
...
def createClassCPP(path, classname):
    ...
    if (not re.match(".*/test", path)):
        context = context + "#include <tnet/" + hp + "/" + classname + ".h>" + "\n"
    else:
        context = context + "#include <tnet/" + hp + "/test/" + classname + ".h>" + "\n"
    context = context + "" + "\n"
    context = context + "TNET_BEGIN_NAMESPACE(" + ns + ");" + "\n"
    context = context + "" + "\n"
